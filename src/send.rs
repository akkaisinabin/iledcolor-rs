...
use crate::{ble, image::Image, packet};
use log::debug;
...
pub async fn image(device: Device, image: Image) -> Result<(), std::io::Error> {
    let services = device.discover_services_with_uuid(ble::WRITE_SERVICE_UUID).await.expect("Service discovery failed");
    ...
    let cmd_char = chars.iter().find(|c| c.uuid() == ble::CMD_CHARIC_UUID).expect("Characteristic 0 not found");
    let write_char = chars.iter().find(|c| c.uuid() == ble::WRITE_CHARIC_UUID).expect("Characteristic 1 not found");
    let notify_char = chars.iter().find(|c| c.uuid() == ble::NOTIFY_CHARIC_UUID).expect("Notify Characteristic not found");
